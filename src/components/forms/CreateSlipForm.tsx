...
export function CreateSlipForm({ onSuccess }: { onSuccess: (slip?: Slip) => void }) {
  ...
  return (
    <form onSubmit={handleCreate} className="p-4 pb-24 md:pb-4 space-y-6">
      {/* Section: Vehicle & Driver */}
      <div className="space-y-3">
        <h3 className="text-sm font-bold text-zinc-900 dark:text-white flex items-center gap-1.5">
          <Truck className="w-4 h-4 text-zinc-500" /> Vehicle & Driver
        </h3>
        <div>
      ...
      {/* Section: Measurement */}
      <div className="space-y-3">
        <h3 className="text-sm font-bold text-zinc-900 dark:text-white">Measurement</h3>
        <div>
      ...
      <div className="space-y-3">
        <h3 className="text-sm font-bold text-zinc-900 dark:text-white">Customer & Pricing</h3>
        <div>
      ...
      {/* Section: Payment */}
      <div className="space-y-3">
        <h3 className="text-sm font-bold text-zinc-900 dark:text-white">Payment</h3>
        <div>
        ...
        <div className="bg-zinc-50 dark:bg-zinc-800/60 rounded-xl border border-zinc-200 dark:border-zinc-700 p-3 space-y-1">
          <div className="flex justify-between text-sm">
            <span className="text-zinc-500">Quantity</span>
            <span className="font-semibold text-zinc-900 dark:text-white">{calculatedQty.toFixed(2)} {formData.measurementType === "Volume (Brass)" ? "Brass" : "T"}</span>
          </div>
          <div className="flex justify-between text-sm">
            <span className="text-zinc-500">Rate</span>
            <span className="font-semibold text-zinc-900 dark:text-white">₹{formData.ratePerUnit}</span>
          </div>
          <div className="flex justify-between text-base pt-1 border-t border-zinc-200 dark:border-zinc-700">
            <span className="text-zinc-700 font-medium">Total</span>
            <span className="font-bold text-primary-600 dark:text-primary-400">₹{finalAmount.toLocaleString()}</span>
          </div>
        </div>
      </div>
      ...
      <div className="md:hidden fixed bottom-0 left-0 right-0 bg-white/95 dark:bg-zinc-950/95 backdrop-blur-md border-t border-zinc-200 dark:border-zinc-800 p-3 pb-[calc(0.75rem+env(safe-area-inset-bottom))] flex items-center gap-2 z-50">
        ...
        >
          + Create Slip · ₹{finalAmount.toLocaleString()}
        </button>
      </div>
    </form>
  );
